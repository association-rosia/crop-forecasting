...
def init_pipeline() -> Pipeline:
    params_pipeline = {
        "smoother__mode": wandb.config.smoother,
        "convertor__agg": wandb.config.aggregate,
        "convertor__weather": wandb.config.weather,
        "convertor__vi": wandb.config.vi,
    ...
    if wandb.config.pca:
        ...
        steps_pipeline.append(
            ("dim_reductor", PCA(n_components=wandb.config.n_components))
        )
