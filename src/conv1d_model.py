...
def main():
    ...
    wandb.config['s_sequence_length'] = first_batch['s_input'].shape[0]
    wandb.config['m_sequence_length'] = first_batch['m_input'].shape[0]
    wandb.config['g_in_features'] = first_batch['g_input'].shape[0]
    wandb.config['c_in_features'] = 482

    model = Conv1dModel(wandb.config)
    model.to(DEVICE)
...
class Conv1dModel(nn.Module):
    def __init__(self, config):
        ...
        self.s_in_channels = config['s_sequence_length']
        self.m_in_channels = config['m_sequence_length']
        self.out_channels = 20
        
        self.s_cnn = nn.Conv1d(self.s_in_channels, self.out_channels, kernel_size=3)
        self.m_cnn = nn.Conv1d(self.m_in_channels, self.out_channels, kernel_size=3)
